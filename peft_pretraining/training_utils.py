...
def detect_grad_spikes(optimizer, threshold=50):
    ...
    # select the max magnitude spike
    gradient_spikes = torch.tensor(gradient_spikes)
    idx_max_grad = torch.argmax(torch.abs(gradient_spikes))
    gradient_spike = gradient_spikes[idx_max_grad]
